  ...
  <div class="container">
    <div class="checkout-addr">
      ...
      <div class="addr-list-wrap">
        <div class="addr-list">
          <ul>
            ...
            :key="index" 
            @click="setAddressCur(index,item.addressId)" 
            :class="{'check':index==curAddressIndex||item.isDefault}"
            >
              <dl>
                <dt>{{item.userName}}</dt>
              ...
              <div class="addr-opration addr-del">
                ...
                @click.stop="delAddress(item.addressId)"
                >
              ...
              <div class="addr-opration addr-set-default">
                ...
                v-if="!item.isDefault && defaultIndex!=index"
                @click="setDefaultAddress(item.addressId)"
                ><i>Set default</i></a>
              </div>
              <div class="addr-opration addr-default" v-show="showDefault" v-if="item.isDefault || defaultIndex==index">Default address</div>
            </li>
            <li class="addr-new">
        ...
        <div class="shipping-addr-more">
          <a class="addr-more-btn up-down-btn" href="javascript:;" @click="showAllAddress" :class="{'open':limit>3}">
            more
            <i class="i-up-down">
      ...
      <div class="next-btn-wrap">
        ...
        :to="{path:'orderConfirm',query:{addressId:selectAddId}}"
        >
        NEXT
        </router-link>
      </div>
...
<modal :mdShow="modalConfirm" @close="closeModal">
    ...
    <div slot="btnGroup">
    <a class="btn btn--m" href="javascript:;" @click="confirmDelAddress">确认</a>
...
export default {
  ...
  methods: {
    ...
    setAddressCur(index, addressId) {
      if (index != this.defaultIndex) {
        this.showDefault = false;
      } else {
        this.showDefault = true;
      }
      this.selectAddId = addressId;
    ...
    setDefaultAddress(addressId) {
      ...
      this.$http({
        ...
        method: "POST",
        data: {
          addressId: this.setAddId
        }
      }).then(response => {
        let res = response.data;
        if (res.status == ERR_OK) {
          this.defaultIndex = this.setAddId;
          this.showDefault = true;
          this.getAddressList();
        }
      });
